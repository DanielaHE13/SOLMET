...
/* ---- Autorización (solo admin puede editar) ---- */
...
$URL_EDITAR = '?pid=' . base64_encode('PRESENTACION/Maquina/editar.php');

/* ---- ID target ---- */
$id = trim($_GET['id'] ?? '');
if ($id === '' || !is_valid_id($id)) {
  echo err('ID de máquina inválido.'); echo '<div class="p-3"><a class="btn btn-secondary" href="'.h($URL_LISTAR).'">Volver</a></div>'; exit;
}
...
$registro = null;
try {
  $cx->ejecutar("SELECT id_maquina, nombre, estado, updated_at FROM maquina WHERE id_maquina = ?", [$id]);
...
if (!$registro) {
  $cx->cerrar();
  echo err('La máquina solicitada no existe.'); echo '<div class="p-3"><a class="btn btn-secondary" href="'.h($URL_LISTAR).'">Volver</a></div>'; exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!hash_equals($csrf, $tkn)) {
    ...
    if (!$errors) {
      try {
        // Comprobar concurrencia optimista (alguien más modificó)
        $cx->ejecutar("SELECT updated_at FROM maquina WHERE id_maquina = ?", [$id]);
        $row = $cx->registro();
        $currUp = $row['updated_at'] ?? null;
        $currUpStr = $currUp ? (string)$currUp : '';

        if ($currUpStr !== $prevUp) {
          $errors[] = 'La máquina fue modificada por otro usuario. Vuelve a cargar y reintenta.';
        } else {
          // Actualizar
          $cx->ejecutar("UPDATE maquina SET nombre = ?, estado = ? WHERE id_maquina = ?", [$nombre, $estado, $id]);
...
$valEstado = $_POST['estado'] ?? ($registro['estado'] ?? 'activa');
$valUpdAt  = $registro['updated_at'] ?? null; // puede ser null
$valUpdStr = $valUpdAt ? (string)$valUpdAt : '';
?>
<!doctype html>
<html lang="es">
<head>
  ...
  <div class="hero mb-3">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
      <div>
        ...
        <h3 class="hero-title mb-0">Editar máquina</h3>
        <div class="mt-1 text-muted small">ID: <span class="fw-semibold"><?= h($registro['id_maquina']) ?></span></div>
      </div>
